...
public class Solver3States extends Solver {
    ...
    public int[][] getAllSolutions(int[][] configMatrix) {
        ...
        int[][] gaussJordanMatrix = this.makeJordanGaussElim(getAugmentedMatrix(getLightsOutMatrix(size), configMatrix));

        if (getRank(getLightsOutMatrix(size)) == getRank(getAugmentedMatrix(getLightsOutMatrix(size), configMatrix))) {
            solutions = this.getQuietPatterns(size); // generate quiet patterns for
                                                // the size of game
    ...
    public int[][] getQuietPatterns(int size) {
        int[][] matrix; // matrix of lights out matrix, to be changed with
                        // Gauss-Jordan elimination
        int nullity; // the dimension of the null space of the lights out matrix
        int[][] quietPatterns;

        matrix = this.makeJordanGaussElim(getLightsOutMatrix(size));
        ...
        // find all combinations of dependent variables in Finite Field of size
        // 2
    ...
    public int[][] makeGaussElimination(int[][] matrix) {
        ...
        GaussElim: {
            int q = 0; // initialize column number
            for (int p = 0; (p < matrix.length) && (q < matrix[p].length); p++, q++) {
                // p is row number
    ...
    public int[][] makeJordanGaussElim(int[][] matrix) {
        matrix = this.makeGaussElimination(matrix);
        ...
        for (int p = matrix.length - 1; p > 0; p--) {
            ...
            if (!flag) {
                ...
                // make all elements above the first non-zero element of p-th
                // row
                // equals to 0:
                for (int i = p - 1; i >= 0; i--) {
                    if (matrix[i][q] != 0) {
